...
ap.add_argument('--busybox', type=Path, default=ROOT.parent / 'omarchy-oneplus7pro/.work/codex-native-initramfs/bin/busybox')
...
expected_base = (ROOT / 'devices/pixel7pro/kernel/kernel-base.txt').read_text().strip()
if base != expected_base:
    ap.error('Kernel base differs from the saved v9 checkpoint')
if a.acpm:
    overlay = subprocess.check_output(['dtc', '-@', '-I', 'dts', '-O', 'dtb',
                                      str(ROOT / 'mainline/pixel-acpm-overlay.dts')])
    (kernel / 'arch/arm64/kernel/pixel_acpm_overlay.h').write_text(
        '/* SPDX-License-Identifier: GPL-2.0-only */\n'
        '/* Generated from mainline/pixel-acpm-overlay.dts */\n'
        'static const unsigned char pixel_acpm_overlay[] __aligned(8) = {\n' +
...
shutil.copy2(ROOT / 'devices/pixel7pro/kernel/native-bringup-v9.config', kernel / '.config')
subprocess.run([str(kernel / 'scripts/config'), '--file', str(kernel / '.config'),
                ...
                '--set-str', 'LOCALVERSION', '-pixel-power14' if a.acpm else ('-pixel-drm11' if a.drm else ('-pixel-net10' if a.usb_network else '-pixel-shell9')),
