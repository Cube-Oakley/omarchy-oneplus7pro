/* Hardware-only offscreen GLES2 shader/readback test. Opens a DRM node for GBM; never modesets.
...
int main(int argc, char **argv)
{
    ...
    destroy_gbm(device);
    close(fd);
    puts(pass ? "PASS: Mali-G710 shader rendered and pixels verified" : "FAIL: unexpected pixels");
    return pass ? 0 : 1;
}
