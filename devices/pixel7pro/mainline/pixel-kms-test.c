...
/* Standard DRM userspace test: dumb buffers, modeset, page-flip events.
...
static struct buffer make_buffer(int fd, unsigned int phase)
{
    ...
    for (unsigned int y=0; y<3120; y++)
        for (unsigned int x=0; x<1440; x++) {
            ...
            if (y>=1250 && y<1700) color=colors[x/480];
            if (y>=2100 && y<2420) color=((x/80+y/80+phase)&1)?0xffffffff:0xff101820;
            if (x>=80+phase*640 && x<600+phase*640 && y>=2600 && y<2800) color=0xffffbb33;
            b.map[y*(b.pitch/4)+x]=color;
        }
    text(&b, 80, 650, "PIXEL 7 PRO", 0xffffffff);
    text(&b, 80, 780, "NATIVE LINUX / DRM", 0xff80dfcf);
    text(&b, 80, 950, "1440 x 3120", 0xffffffff);
    text(&b, 80, 1080, "USB SHELL + DISPLAY", 0xffffffff);
    text(&b, 80, 1810, "RED       GREEN       BLUE", 0xffffffff);
    text(&b, 80, 1950, phase ? "PAGE FLIP B" : "PAGE FLIP A", 0xffffffff);
...
{
    unsigned int seconds = argc>1 ? strtoul(argv[1],NULL,10) : 300;
    if (seconds>3600) { fputs("duration must be <=3600\n",stderr); return 1; }
    ...
    struct buffer b[2]={make_buffer(fd,0),make_buffer(fd,1)};
    ...
    for(unsigned int i=1;i<=8;i++) {
        sleep(2);
        struct drm_mode_crtc_page_flip flip={.crtc_id=crtc,.fb_id=b[i&1].fb,.flags=DRM_MODE_PAGE_FLIP_EVENT,.user_data=i};
        ...
        printf("PIXEL_KMS_FLIP_OK frame=%u event=%u sequence=%u\n",i,event.base.type,event.sequence);
    }
    printf("PIXEL_KMS_TEST_OK; holding test card for %u seconds\n",seconds);
    sleep(seconds);
    return 0;
}
