#!/busybox sh
# Flash 50: 49 plus PHY only if 13-0025/driver exists. Delay +20 adapter
# +20 TCPC driver. No i2c autoprobe=1 (48 looped).
BB=/busybox
...
: > "$TMPLOG" 2>/dev/null
: > "$LOG" 2>/dev/null
log "flash=50 cap=100 tcpc-driver-then-phy ACM-or-rebootbl"
log "init pid=$$"
...
# insmod i2c-exynos5 probes all 7 HSI2C and hung (38/39). Autoprobe off so
# only 10d60000.hsi2c is bound. Do not ls/cat other *.hsi2c sysfs (40/41).
log "fork i2c then tcpc-bg"
(
  $BB echo 0 > /sys/bus/platform/drivers_autoprobe 2>/tmp/w.err \
    && log "plat autoprobe=0" || log "plat ap $($BB cat /tmp/w.err)"
  $BB echo 0 > /sys/bus/i2c/drivers_autoprobe 2>/tmp/w.err \
    && log "i2c autoprobe=0" || log "i2c ap $($BB cat /tmp/w.err)"
  log "child insmod i2c-exynos5"
  insmod_one i2c-exynos5.ko
  $BB echo 10d60000.hsi2c > /sys/bus/platform/drivers/exynos5-hsi2c/bind 2>/tmp/hsi.err &
  log "bind hsi13 launched"
  wait_s 8
  log "child adr=$($BB ls /sys/class/i2c-adapter 2>/dev/null | $BB tr '\n' ' ')"
  $BB echo 13-0025 > /sys/bus/i2c/drivers/max77759tcpc/bind 2>/tmp/tcpc.err &
  log "bind tcpc launched"
  wait_s 8
  log "child 13-0025=$(test -e /sys/bus/i2c/devices/13-0025 && echo y || echo n) drv=$(test -e /sys/bus/i2c/devices/13-0025/driver && echo y || echo n)"
  if [ -e /sys/bus/i2c/devices/13-0025/driver ]; then
    $BB echo 11200000.phy > /sys/bus/platform/drivers/phy_exynos_usbdrd/bind 2>/tmp/phy.err &
    $BB echo 11210000.usb > /sys/bus/platform/drivers/exynos-dwc3/bind 2>/tmp/dwc.err &
    log "bind phy/dwc3 launched"
    wait_s 8
  else
    log "skip phy, no tcpc driver"
  fi
) &
log "child pid=$!"
wait_s 30
...
log "no real UDC — delay telemetry then rebootbl"
extra=0
[ -n "$($BB ls /sys/class/i2c-adapter 2>/dev/null)" ] && extra=$((extra + 20))
[ -e /sys/bus/i2c/devices/13-0025/driver ] && extra=$((extra + 20))
log "telemetry extra=$extra tcpc_drv=$(test -e /sys/bus/i2c/devices/13-0025/driver && echo y || echo n)"
wait_s $((10 + extra))
exec /rebootbl
