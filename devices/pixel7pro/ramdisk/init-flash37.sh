...
# trybind/gadget/PHY probe can freeze PID 1. Idle so a hold-through
# (Power+Vol Down through the logo) keeps ramoops, like flash 14.
log "DUMP DONE — idle hold Power+VolDown through logo"
while :; do
  wait_s 30
  log "idle udc=$($BB ls /sys/class/udc 2>/dev/null | $BB tr '\n' ' ') phy=$(test -d /sys/module/phy_exynos_usbdrd_super && echo y || echo n)"
done
