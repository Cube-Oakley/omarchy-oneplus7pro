...
/*
 * Samsung S5K3M5, the OnePlus 7 Pro's rear telephoto: the linux-next driver
 * as the OnePlus 7T Pro port carries it (hotdog r181: 0058, 0081). Changed
 * for guacamole (docs/camera-20260922.md): the supplies come up one at a
 * time in the order listed, analogue before digital, and go down in reverse,
 * where a bulk enable raced them (on this board's Sony slots a digital rail
 * first tripped the PMIC's undervoltage lockout); and the sensor links its
 * focus actuator ("lens-focus"), so the lens powers with it.
 */
...
struct s5k3m5_mode {
	...
	struct v4l2_rect crop;		/* Analogue crop before binning */
...
/* Supplies up one at a time in s5k3m5_supply_names order, down in reverse. */
static void s5k3m5_supplies_off(struct s5k3m5 *s5k3m5, int count)
{
	while (--count >= 0)
		regulator_disable(s5k3m5->supplies[count].consumer);
}

static int s5k3m5_supplies_on(struct s5k3m5 *s5k3m5)
{
	int i, ret;

	for (i = 0; i < S5K3M5_NUM_SUPPLIES; i++) {
		ret = regulator_enable(s5k3m5->supplies[i].consumer);
		if (ret) {
			s5k3m5_supplies_off(s5k3m5, i);
			return ret;
		}
...
disable_regulators:
	s5k3m5_supplies_off(s5k3m5, S5K3M5_NUM_SUPPLIES);
...
/* Power the focus actuator ("lens-focus") whenever the sensor is powered. */
static void s5k3m5_link_lens(struct s5k3m5 *s5k3m5)
{
	struct device_node *np;
	struct i2c_client *lens;

	np = of_parse_phandle(s5k3m5->dev->of_node, "lens-focus", 0);
	if (!np)
		return;
	lens = of_find_i2c_device_by_node(np);
	of_node_put(np);
	if (!lens) {
		dev_warn(s5k3m5->dev, "focus actuator device not found\n");
		return;
	}

	s5k3m5->lens_link = device_link_add(s5k3m5->dev, &lens->dev,
					    DL_FLAG_STATELESS | DL_FLAG_PM_RUNTIME);
	put_device(&lens->dev);
	if (!s5k3m5->lens_link)
		dev_warn(s5k3m5->dev, "could not link the focus actuator\n");
}
...
static int s5k3m5_probe(struct i2c_client *client)
{
	...
	s5k3m5->mclk = devm_clk_get(s5k3m5->dev, NULL);
...
subdev_cleanup:
	...
	if (s5k3m5->lens_link)
		device_link_del(s5k3m5->lens_link);
