...
# devices/oneplus7pro/adapter/camera/libcamera, and with "install" replaces Arch's
...
SRC="$ROOT/adapter/camera/libcamera"
...
tar czf - -C "$SRC" PKGBUILD imx586.yaml s5k3m5.yaml imx481.yaml $(cd "$SRC" && ls 00*.patch) -C "$(dirname "$TARBALL")" "$(basename "$TARBALL")" |
