...
# the Sony IMX586 main-camera driver, the Samsung S5K3M5 telephoto driver
# with the v4l2-cci helper it uses (our .config leaves V4L2_CCI unset, so it is
# built here with its I2C half), and the Sony IMX481 ultra-wide driver. The phone has no module tree for this
...
rm -rf "$WORK"
mkdir -p "$WORK/cci" "$WORK/imx586" "$WORK/s5k3m5" "$WORK/imx481" "$OUT"
copy() {
...
cp "$SRC/imx481.c" "$WORK/imx481/"
printf 'obj-m += imx481.o\n' > "$WORK/imx481/Makefile"
...
cp "$WORK"/mc/mc.ko "$WORK"/v4l2-core/{videodev,v4l2-async,v4l2-fwnode,v4l2-cci}.ko \
    ...
    "$WORK"/s5k3m5/s5k3m5.ko "$WORK"/imx481/imx481.ko "$OUT/"
...
printf '%s\n' mc videodev v4l2-async v4l2-fwnode v4l2-cci videobuf2-common videobuf2-memops \
    videobuf2-v4l2 videobuf2-dma-sg qcom-camss i2c-qcom-cci imx586 s5k3m5 imx481 > "$OUT/load-order"
