...
TEST = ROOT / '.work/linux-sm8150-cx-sleep'
OUT = ROOT / 'out/cx-sleep-test'
FROZEN = ROOT / 'out/checkpoints/20260917-native5-test'


def main():
    with tempfile.TemporaryDirectory(prefix='cx-image-check-') as tmp:
        ...
        assert b'guacamole_cx_sleep_release' not in (paths[0] / 'kernel').read_bytes()
    ...
    print('Runtime default-off boot and controlled CX sleep trial remain pending')
