#!/usr/bin/env bash
# Flash the checked native3 charging test, or its native2 rollback, on slot B.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/scripts/phone-config.sh"
SERIAL=$(phone_serial)
TEST="$ROOT/out/checkpoints/20260917-native3-test"
ROLLBACK="$ROOT/out/checkpoints/20260917-native2-verified"
case "${1:-test}" in
    test) IMAGE="$TEST" ;;
    rollback) IMAGE="$ROLLBACK" ;;
    *) echo 'Usage: flash_power_desktop.sh [test|rollback]' >&2; exit 2 ;;
esac
