...
| Board start (modules, address, `bluetoothd`) | `devices/oneplus7pro/adapter/bluetooth/start.sh` → `/usr/local/sbin/guacamole-bluetooth-start` |
...
- Autostart after the desktop: `touch /root/bluetooth-bringup/autostart-enabled`
  (hook in `devices/oneplus7pro/adapter/desktop-prepare.sh`). Enabled on the phone at
