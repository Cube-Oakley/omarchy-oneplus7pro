...
separately from the shell, and the [Pixel 7 Pro](../pixel7pro/README.md) runs the same interface.
...
This directory is the OnePlus 7 Pro workspace inside the
[Omarchy Mobile](../../README.md) repository. The shell itself is shared and
lives in [`overlay/mobile/`](../../overlay/mobile/README.md); its tests are in
the repository's top-level `tests/`. Paths below are relative to this directory.

```text
adapter/                guacamole rootfs adapter: session setup, power, audio, sensors
kernel/                 kernel patches, device-tree overlays and boot parameters
scripts/                build, transfer, guarded flashing and diagnostic tools
tests/                  hardware-policy checks
docs/                   results, limitations and experiment history
