...
for stage in radio-control wifi; do
    ...
    mkdir -p "$WORK"
    dtc -@ -I dts -O dtb "$ROOT/devices/oneplus7pro/kernel/power/guacamole-$stage.dts" \
        -o "$OUT/$stage.dtbo"
    fdtoverlay -i "$OUT/$NAME.dtb" -o "$OUT/$stage-merged.dtb" "$OUT/$stage.dtbo"
    cp "$ROOT/devices/oneplus7pro/kernel/power/radio_overlay.c" "$WORK/radio_overlay.c"
