#!/usr/bin/env bash
# native4 radio preflight, with the charging-capable native3 rollback.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/scripts/phone-config.sh"
SERIAL=$(phone_serial)
TEST="$ROOT/out/checkpoints/20260917-native4-test"
ROLLBACK="$ROOT/out/checkpoints/20260917-native3-verified"
case "${1:-test}" in
    test) IMAGE="$TEST" ;;
    rollback) IMAGE="$ROLLBACK" ;;
    *) echo 'Usage: flash_wifi_desktop.sh [test|rollback]' >&2; exit 2 ;;
esac
