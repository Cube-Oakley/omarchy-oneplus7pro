#!/usr/bin/env bash
# Build-only: the camera DT overlays (CCI0, CAMSS, PM8009 rails and one slot:
# the IMX586 main camera or the S5K3M5 telephoto) and their loader modules
# for the running native5 kernel. Checked
...
# Both rear cameras, once each works on its own (docs/camera-telephoto-20260924.md).
variant guacamole_camera_rear "CAMERA_MAIN CAMERA_TELE" IMX586,S5K3M5
...
(cd "$OUT" && sha256sum guacamole_camera.ko camera.dtbo guacamole_camera_tele.ko camera-tele.dtbo \
    guacamole_camera_rear.ko camera-rear.dtbo > SHA256SUMS)
