#!/usr/bin/env bash
# Flash only slot B's boot image. Default-off MMCX diagnostic; preserve the DSI fix.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.." && pwd)
source "$ROOT/scripts/phone-config.sh"
SERIAL=$(phone_serial)
TEST="$ROOT/out/checkpoints/20260918-mmcx-sleep-test"
ROLLBACK="$ROOT/out/checkpoints/20260917-dsi-phy-pm-test"
case ${1:-test} in
    test) IMAGE="$TEST" ;;
    rollback) IMAGE="$ROLLBACK" ;;
    *) echo 'Usage: flash_mmcx_sleep_test.sh [test|rollback]' >&2; exit 2 ;;
esac
