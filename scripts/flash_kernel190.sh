#!/usr/bin/env bash
# Flash only slot B's boot image: #190 (#189 plus the hsuart0 alias and PM8150L GPIOs), or #189 back.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.." && pwd)
source "$ROOT/scripts/phone-config.sh"
SERIAL=$(phone_serial)
TEST="$ROOT/out/checkpoints/20260923-kernel190"
ROLLBACK="$ROOT/out/checkpoints/20260922-ipa-kernel189-test"
case ${1:-test} in
    test) IMAGE="$TEST" ;;
    rollback) IMAGE="$ROLLBACK" ;;
    *) echo 'Usage: flash_kernel190.sh [test|rollback]' >&2; exit 2 ;;
esac
