#!/usr/bin/env bash
# Flash only slot B's boot image: #193 (#192 with the OEM carve-out map and
# the FastRPC pool the sensor DSP needs; scripts/build_kernel193.sh), or #192
# back.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.." && pwd)
source "$ROOT/scripts/phone-config.sh"
SERIAL=$(phone_serial)
TEST="$ROOT/out/checkpoints/20260924-kernel193"
ROLLBACK="$ROOT/out/checkpoints/20260923-kernel192"
case ${1:-test} in
    test) IMAGE="$TEST" ;;
    rollback) IMAGE="$ROLLBACK" ;;
    *) echo 'Usage: flash_kernel193.sh [test|rollback]' >&2; exit 2 ;;
esac
