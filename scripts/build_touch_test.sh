...
patch --batch -d "$WORK" -p1 < "$ROOT/devices/oneplus7pro/kernel/touch/s6sy761-reset.patch"
patch --batch -d "$WORK" -p1 < "$ROOT/devices/oneplus7pro/kernel/touch/s6sy761-guacamole-probe.patch"
patch --batch -d "$WORK" -p1 < "$ROOT/devices/oneplus7pro/kernel/touch/s6sy761-resume-irq.patch"
...
cp "$ROOT/devices/oneplus7pro/kernel/touch/touch_overlay.c" "$WORK/"
dtc -@ -I dts -O dtb "$ROOT/devices/oneplus7pro/kernel/touch/guacamole-touch.dts" -o "$OUT/guacamole-touch.dtbo"
