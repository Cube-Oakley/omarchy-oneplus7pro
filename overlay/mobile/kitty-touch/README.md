...
Phone backup: `/root/backups/pre-gestures-20260917/glfw-wayland.so`.
Restore it using the same stage-and-rename approach, then reopen terminals.
...
Limits: touch text selection, long-press actions and multi-finger gestures are
not implemented. The patch forwards scrolling through Kitty's usual path,
so applications that capture scrolling decide their own behavior. Terminal
